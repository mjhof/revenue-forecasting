...
## 1.2 Log transform interim_sales variable
```{r}
par(mfrow = c(2, 1))
...
## 1.3 Remove duplicated data points
```{r}
...
## 1.4 Exclude companies with less than 40 observations
```{r}
...
## 1.5 Interpolate companies with non-continous time series'
```{r}
...
## 1.6 Create a seperate column for the sales variable in each quarter
```{r}
...
# 2.2 Casefold variable names and replace whitespaces
```{r}
...
## 2.3 Remove variables where > 20% of values are missing
...
## 2.4. Log transform right-skewed variables
...
# 3.2 Casefold variable names and replace whitespaces
```{r}
...
## 3.3 Remove variables where > 20% of values are missing
...
```
## 3.4. Log transform right-skewed variables
...
## 3.5. Impute missing values for other variables with spline interpolation
While performing interpolation, we found out, that there are certain variables that are completely missing for certain companies. We have to options on how to deal with this issue: The first one is to remove those companies from the dataset, the second one is to remove those variables from the dataset. As we have many companies in the dataset, we will remove those from the dataset.
...
## 4.1 Fuzzy join company names of sales and balance sheet data frame
```{r}
...
## 4.2 Manually resolve companies that could not be matched
By looking at the results above, we can see that the matching strategy worked, but that there are still some companies, where we have to perform the matching manually (e.g., 3M COMPANY).
...
companies_joined_best[
  companies_joined_best$company.x == "NIKE INC.", "company.y"] <- "NIKE 'B'"
...
## 4.3 Examine if balance sheet and profit and loss data frames share the same company naming scheme
```{r}
...
df_pl_companies <- read_csv(PROFIT_LOSS_PATH, show_col_types = FALSE)
df_pl_companies <-
...
if (length(setdiff(
  ...
  cat("Balance sheet data frame and profit and loss data frame share same company naming scheme.")
} else {
  cat("Balance sheet and profit and loss data frame have different company naming scheme.")
}
```

## 4.5 Join sales, balance sheet and profit and loss to one data frame
```{r}
...
df_joined
```
## 4.6 Write joined data to file
```{r}
write.csv(df_joined, DATA_JOINED_OUTPUT_PATH)
```


# 5. Joint data analysis and preparation 
## 5.1 Shift target (sales) variables backwards
...
df_joined_shift <- df_joined %>%
  ...
  na.omit() # the back shifting will create NA values for the last year in each series, we will omit those
```
...
corr_cross(
  df_joined_shift %>% select( # exclude target variables, year and company from analysis
    -c(
      index,
      company.sales,
      company.rest,
...
## 5.3 Perform variable selection to remove highly correlated pairs
```{r}
...
## 5.4 Perform dimensionality reduction with Principal Component Analysis (PCA)
```{r}
...
The first 20 principal component capture 95.2% of variance of the data, this should be sufficient for  modelling and reduces the dimensionality from 56 to 27 compared to our variable selection approach. 
...
## 5.4 Show overview of processed data sets
```{r}
cat(paste(
...
## 5.5 Save unique companies of joined data frame to enrich it with industry information with OpenRefine
```{r}
...
## 5.6 Add industry information from WikiData with Openrefine
This step was performed within Google OpenRefine. OpenRefine offers the possibility to import a data set and to link it with knowledge bases like WikiData. To achieve linkage (reconciliation) between the companies, the company names are used. For some companies it was neccessary to resolve them manually. After the reconciliation is done, we can add new variables from the data that is present on WikiData. In this case, we will use the top 3 industries, that can be found. This information will later be used when evaluting the models. Here we will examine if the prediction quality differs for different industries/business sectors. Down below, you can see the the results we obtained from OpenRefine.

![OpenRefine-Data-Enrichment](../images/openrefine-5.png){width="100%", height="100%"}

# 6. Train test split
## 6.1 Retrieve train and test indices
```{r}
...
indices_train <- setdiff(df_joined_shift_var_sel$index, indices_test)
...
# train
write.csv(
  df_joined_shift_var_sel %>% filter(index %in% indices_train),
  TRAIN_VAR_SEL_OUTPUT_PATH
)
# test
write.csv(
  df_joined_shift_var_sel %>% filter(index %in% indices_test),
...
# train
write.csv(
  df_joined_shift_pca %>% filter(index %in% indices_train),
  TRAIN_PCA_OUTPUT_PATH
)
# test
write.csv(
  df_joined_shift_pca %>% filter(index %in% indices_test),
